...
Para ejecutar dicho programa ha sido necesario instalar phyton3. Este programa usa el fichero batch_score.py que se tiene que ejecutar de la siguiente manera: 
batch_score.py -c <config> -r <repos_path> -m <manifest> -s <sample_file>

donde 

\begin{itemize}
\item <config>: es una parte de config.json.

\item <repos_path>: es la dirección de un directorio donde el programa puede comprobar los ficheros fuente de un proyecto.

\item <manifest>: es una parte de manifest.json que contiene información sobre qué atributos deben ejecutarse. 

\item <sample_file>: es una lista de los identificadores de los proyectos de GHTorrent que se van a analizar.
\end{itemize}

A continuación el fichero config.json contiene las siguientes claves y valores:

\begin{itemize}
\item Thershold (define el umbral por el cual el sistema considera que un repositorio contiene un proyecto de software): es un número positivo.

\item persistResult (decide si los resultados obtenidos deben guardarse en la fuente de datos especificada): true or false.

\item Datasource (Configuración para conectarse a la base de datos de GHTorrent): object.

\item github_tokens (es una lista de los identificadores de los autores): list.
\end{itemize}
