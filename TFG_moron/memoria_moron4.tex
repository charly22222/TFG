...
\vspace{1cm}
\large
Aplicación de Big Data a Proyectos de Ingeniería en itHub
...
En el mundo actual centrado en el software, repositorios de software a gran escala, p. ej. SourceForge (más de 324.000 proyectos) y GitHub (más de 69.000.000 proyectos) son la nueva biblioteca de Alejandría. Contienen una enorme cantidad de software e información sobre el software. Tanto los científicos como los ingenieros están interesados en analizar esta gran cantidad de información tanto por curiosidad como por probar sus hipótesis. Sin embargo, la extracción sistemática de datos relevantes de estos repositorios y el análisis de dichos datos para probar hipótesis es difícil.\\
...
De esta forma se ha realizado un muestreo de 2.316.524 repositorios y vemos que en los repositorios de propósito general el clasificador basado en puntuación ha identificado un 71,02\% de proyectos de ingeniería, mientras que Random Forest ha identificado un 23,51\%, por lo que, aunque sobre el papel Random Forest es más preciso, ha dado peor resultado, quizá debido a que como existe una correlación entre las dimensiones se favorecen los grupos más pequeños sobre los más grandes. 
...
In this way, 2.316.524 repositories have been sampled and we see that in the general purpose repositories, the score-based classifier has identified 71.02 \% of engineering projects, while Random Forest has identified 23.51 \%, so even though on the Random Forest paper it is more precise, it has given a worse result, perhaps because, as there is a correlation between the dimensions, the smaller groups are favored over the larger ones.
...
Debido a la gran cantidad de datos que hay que almacenar y tratar, las tecnologías Big Data han estado aparentemente monopolizadas por grandes corporaciones, pero cada vez se avanza más en su aplicación en Universidades e instituciones con una gran I+D implantada, así como en los departamentos I+D de empresas tecnológicas ~\cite{Yeray}\\
...
Hemos analizado estos repositorios porque son gratuitos y de libre acceso, y son los más utilizados actualmente, aunque han ido surgiendo otros como GitLab, que surgió a raíz de la compra de GitHub por Microsoft y ante el temor de los desarrolladores de software a que se limitara el acceso, llegando esta plataforma a alojar más de 546.000 proyectos a finales de 2017.


\section{Retos del Big Data}
\label{sec:retos}

Como ya se ha comentado, el concepto de Big Data lleva aparejado el manejo de una gran cantidad de datos y para ello es necesario disponer de las herramientas conceptuales y matemáticas necesarias para poderlos analizar, ya que cerca del 85\% de esos datos no van a estar bien estructurados y sometidos a errores ~\cite{Yeray}.\\
...
Por otro lado, tenemos el problema añadido de que no sirve cualquier persona para poder realizar el tratamiento de esos datos, sino que tienen que formarse buenos científicos para la gestión de la información derivada del procesamiento de esos datos ~\cite{Batista}. 

\section{Análisis de los datos}
\label{sec:analisis}

Hay que tener en cuenta al poder analizar los datos procedentes de Big Data que estamos hablando de conjuntos de datos cuyo tamaño es del orden de TB, lo cual plantea nuevos retos en el desarrollo y la utilización de plataformas informáticas ~\cite{Dean}, ~\cite{Ghemawat} y ~\cite{Gobioff}.\\ 
...
Mi trabajo fin de grado consiste en identificar los proyectos de ingeniería de los repositorios de software libre alojados en la plataforma GitHub.
...
Además de las siete dimensiones enumeradas anteriormente, es necesario tener en cuenta el tamaño del repositorio, utilizando la métrica de las líneas de código fuente (SLOC), ya que hay que tener en cuenta como dicho tamaño influye en las otras dimensiones. En este trabajo se ha utilizado la utilidad Perl ~\cite{Danial} para recopilar la métrica SLOC de un repositorio (Figura 4.1 \grex{referencia a figura}).
...
Utilidad: estos repositorios se escogieron manualmente de una muestra aleatoria de 2.316.524 repositorios. Se eligieron siguiendo estos criterios: 
...
\bibliography{memoria_moron4}  % memoria.bib es el nombre del fichero que contiene
