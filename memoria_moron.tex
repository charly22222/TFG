...
\vspace{1cm}
La defensa del presente Trabajo Fin de Grado se realizó el día \qquad$\;\,$ de diciembre de 2020, siendo calificada por el siguiente tribunal:
...
\vspace{1cm}
\begin{flushright}
Fuenlabrada, a \qquad$\;\,$ de diciembre de 2020
\end{flushright}
...
El objetivo de este Trabajo Fin de Grado es hacer un estudio sobre las tecnologías Big Data y su aplicación al estudio de proyectos de ingeniería en grandes repositorios como GitHub.\\

Github es un sitio que guarda una carpeta de documentos al que te conectas a través de un terminal de comando (o CMD, en Windows) o por medio de una aplicación de escritorio \grex{repasa esto, que no es la mejor descripción}.\\

En el mundo actual centrado en el software, repositorios de software a gran escala, p. ej. SourceForge (más de 350.000 proyectos), GitHub (más de 250.000 proyectos) y Google Code (más de 250.000 proyectos) son la nueva biblioteca de Alejandría \grex{No sé de dónde sacas estos números, pero no son correctos.}. Contienen una enorme cantidad de software e información sobre el software. Tanto los científicos como los ingenieros están interesados en analizar esta gran cantidad de información tanto por curiosidad como por probar sus hipótesis. Sin embargo, la extracción sistemática de datos relevantes de estos repositorios y el análisis de dichos datos para probar hipótesis es difícil.\\
...
De esta forma se ha realizado un muestreo de 2.316.524 repositorios y vemos que en los repositorios de propósito general el clasificador basado en puntuación ha identificado un 71,02\% de proyectos de ingeniería, mientras que Random Forest ha identificado un 23,51\%. \grex{Y todo esto quiere decir que...}
...
Por otro lado, tenemos el problema añadido de que no sirve cualquier persona para poder realizar el tratamiento de esos datos, sino que tienen que formarse buenos científicos para la gestión de la información derivada del procesamiento de esos datos [50] \grex{La cita...}.
...
Además, para poder analizar dichos datos, es necesario organizarlos y depurarlos adecuadamente para poder sacar la información relevante buscada. Por esto las empresas y diversos organismos oficiales (Universidades, centros de investigación, ministerios...) invierten cada vez más recursos en la formación de profesionales y en investigación y desarrollo [50]. En este último trabajo C. N. \&. Batista \grex{Esta forma de referenciar está mal}, se proponen las fases del análisis que se especifican en la Figura~\ref{figura:fases}.

 \begin{figure}[ht]
    \centering
    \includegraphics[width=12cm, keepaspectratio]{img/Fases.jpg}
    \caption{Fases de análisis.\grex{Si no es tuya, debes comentar la procedencia de la imagen.}}
    \label{figura:fases}
 \end{figure}


\grex{Después de leer tu introducción, no sé de qué va tu proyecto. De hecho, parece no tener nada que ver con lo que comentas en los resultados. Debería quedar claro cuál es el problema, qué has hecho y por qué, motivándolo debidamente.}

\grex{En la intro se debería hacer mención a cómo se definen proyectos de ingeniería, a por qué es importante esa definición y un poco -a vista de pájaro- a qué es lo que has hecho en tu TFG.}
...
Pasos que para mí son logros, ya que he tenido que seguir y conseguir realizar cada uno de esos pasos para poder llegar al final con el trabajo. \grex{Está muy bien, pero yo quitaría esta última frase.}
...
\grex{Todo esto a continuación de GitHub sobra... o si lo pones, hazlo como un párrafo, pero no como una guía.}
...
\begin{figure}[ht]
    ...
    \caption{Imagen extraída de guides.github.com \grex{Falta referenciar imagen en el texto}}
    \label{figura:guides}
 \end{figure}
...
Este Trabajo Fin de Grado está basado en el trabajo de Munaiah y colaboradores (2017), que utilizan una serie de parámetros para identificar los proyectos de ingeniería de software y que se van a describir a continuación. \grex{¡Esto es muy importante! ¡Debería haberse mencionado antes y esta descripción debería estar mucho antes en este capítulo!}

\section{Métricas de un repositorio}

Para identificar los proyectos se van a usar los parámetros: \grex{esto debería haberse puesto en la introducción. De aquí sobra, porque ya lo explicas pormenorizadamente más abajo} Comunidad (evidencia de colaboración), Integración continua (evidencia de calidad), Documentación (evidencia de mantenibilidad), Historia (evidencia de evolución sostenida), Cuestiones (evidencia de la gestión del proyecto, Licencia (prueba de responsabilidad) y Pruebas unitarias (prueba de calidad). Además de las siete dimensiones enumeradas anteriormente, es necesario tener en cuenta el tamaño del repositorio, utilizando la métrica de las líneas de código fuente (SLOC), ya que hay que tener en cuenta como dicho tamaño influye en las otras dimensiones. En este trabajo se ha utilizado la utilidad Perl (Danial, 2014) para recopilar la métrica SLOC de un repositorio (Figura 4.1 \grex{referencia a figura}).
...
\item Problemas: Un ``problema'' en GitHub puede estar asociado con una variedad de etiquetas personalizables que podrían alterar la interpretación del repositorio \grex{???}. 
...
Organización: se han examinado manualmente repositorios de organizaciones conocidas como Amazon, Apache, Facebook, Google y Microsoft y se han elegido manualmente un conjunto de 1.000 repositorios que cumplen alguna de estas características\grex{¿Pero esto lo has hecho tú? Los autores originales miraron 150 ¿cuáles son estos proyectos? Deberían venir en un apéndice}: tiene licencia de código abierto, usa comentarios para documentar el código, usa integración continua y contiene pruebas unitarias. En la figura 4.2 \grex{referencia} se muestra el número de repositorios de cada tipo (``proyecto'' y ``no proyecto'' \grex{Yo los llamaría ``engineered'' y ``non-engineered'', lo de que no sean un proyecto es más difícil de sostener.}) agrupados por lenguaje de programación en la organización y conjuntos de datos de utilidad, donde no proyecto significa en este caso que no son repositorios de organizaciones.
...
\grex{Esta parte necesita más información, sobre machine learning, qué son los clasificadores, y también por qué has elegido estos dos}
...
\grex{Echo en falta hablar de la parte tecnológica. No comentas con qué tecnologías has realizado en análisis. De hecho, me gustaría que hablaras del código en la memoria, y si no está en un repositorio, el código debería ir en un apéndice.}
